...
class NavigationFrame(tk.Frame):
    ...
    def __init__(
        self, parent, load_callback, prev_callback, next_callback, *args, **kwargs
    ):
        ...
        self.next_button = tk.Button(
            self, text="Nächster Schritt", command=next_callback, state=tk.DISABLED
        )
        self.next_button.pack(side=tk.RIGHT, padx=5, pady=10)

        self.prev_button = tk.Button(
            self, text="Vorheriger Schritt", command=prev_callback, state=tk.DISABLED
        )
